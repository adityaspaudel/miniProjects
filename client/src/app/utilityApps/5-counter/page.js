"use client";

import React, { useState } from "react";
import { IoIosAddCircle } from "react-icons/io";
import { GrSubtractCircle } from "react-icons/gr";
import { BiReset } from "react-icons/bi";

const Counter = () => {
  const [count, setCount] = useState(0);

  const increment = () => {
    setCount(count + 1);
  };

  const decrement = () => {
    setCount(count - 1);
  };
  return (
    <div className="flex items-center content-center h-screen w-screen ">
      <div className="flex flex-col gap-10 w-[600px] h-[400px] content-center border-2  items-center m-auto bg-pink-400  text-black rounded-2xl p-8">
        <h2 className="text-black- text-2xl font-bold underline">
          Increment / Decrement Counter
        </h2>
        <button className="text-8xl text-black">{count}</button>
        <div className="flex gap-4 text-white">
          <button
            onClick={increment}
            className="h-12 flex gap-4 justify-center items-center w-40 bg-green-500 hover:bg-green-600 rounded-lg"
          >
            Increment
            <IoIosAddCircle />
          </button>
          <button
            onClick={() => setCount(0)}
            className="h-12 flex gap-4  justify-center items-center w-40 bg-gray-500 hover:bg-gray-600 rounded-lg"
          >
            Reset <BiReset />
          </button>
          <button
            onClick={decrement}
            className="h-12 w-40 flex gap-4 justify-center items-center bg-red-500 hover:bg-red-600 text-center rounded-lg"
          >
            Decrement
            <GrSubtractCircle className="text-center" />
          </button>
        </div>
      </div>
    </div>
  );
};

export default Counter;
